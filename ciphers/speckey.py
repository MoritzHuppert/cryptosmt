...
class SpeckeyCipher(AbstractCipher):
    """
    Represents the differential behaviour of SPECKEY and can be used
    ...
    def createSTP(self, stp_filename, parameters):
        """
        Creates an STP file to find a characteristic for SPECKEY with
        ...
        with open(stp_filename, 'w') as stp_file:
            ...
            w = ["whex{}".format(i) for i in range(rounds)]
            ...
            for i in range(rounds):
                self.setupSpeckeyRound(stp_file, x[i], y[i], x[i+1], y[i+1], w[i],
                                     wordsize)
    ...
    def setupSpeckeyRound(self, stp_file, x_in, y_in, x_out, y_out, w, wordsize):
        """
        Model for differential behaviour of one round SPECKEY
        """
        command = ""
